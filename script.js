...
function drawCanvas() {
    ...
    // ctx.fillStyle = 'rgba(10, 15, 232,0.7)';
    ...
    const albumName = 'niños tristes';
    ctx.fillText(albumName, width/2-ctx.measureText(albumName).width/2, 100*ratio);

    // Drawing artist name
    ctx.fillStyle = 'rgba(255,255,255, 0.7)';
    ctx.font = "16px 'Circular'";
    ctx.fillText('Alvaro Diaz', margin, startingHeight + 450*ratio);

    // Drawing player controls
    ctx.fillStyle = 'rgba(255,255,255, 0.7)';
    ctx.fillRect(margin, startingHeight+480*ratio, imgWidth, 3);
    ctx.beginPath();
    ctx.fillStyle = 'rgb(255,255,255)';
    ...
    ctx.fillText('-3:23', width-margin-23, startingHeight+500*ratio);
    ctx.beginPath();
